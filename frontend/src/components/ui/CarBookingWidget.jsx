...
const CarBookingWidget = ({ car }) => {
  ...
  const { name, phone } = bookingData;
  const { _id: id, price } = car;
  ...
  const totalDays = numberOfDays === 0 && dateRange.from ? 1 : numberOfDays;
  ...
  const initPayment = (data) => {
          ...
          try {
            ...
            const { data: verifyData } = await axiosInstance.post(verifyUrl, {
              ...
              bookingData: {
                ...
                checkOut: dateRange.to || dateRange.from,
                name,
                phone,
                price: totalAmount,
              }
            ...
            if (verifyData.success) {
              toast.success('Payment successful (Sandbox)! Car Booked.');
              setRedirect(`/account/car-bookings/${verifyData.booking._id}`);
            }
          } catch (error) {
    ...
    const options = {
      ...
      description: `Rental for ${car.carName}`,
      order_id: data.id,
      handler: async (response) => {
        try {
          ...
          if (verifyData.success) {
            toast.success('Payment successful! Car Booked.');
            setRedirect(`/account/car-bookings/${verifyData.booking._id}`);
          }
        } catch (error) {
      ...
      theme: {
        color: '#2563EB', // Blue theme for cars
      },
      modal: {
        ondismiss: function () {
          setProcessing(false);
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-xl border border-gray-100 flex flex-col gap-4">
      <div className="flex items-end gap-2 border-b border-gray-100 pb-4">
        <span className="text-2xl font-black text-gray-900">₹{car.price.toLocaleString()}</span>
      ...
      <div className="flex flex-col gap-3 mt-2">
        <button 
          ...
          disabled={processing || totalDays < 1}
        ...
        >
          {processing ? (
            ...
            <>
              ...
              Pay Securely • {totalDays > 0 ? `₹${totalAmount.toLocaleString()}` : 'Select Dates'}
            </>
          )}
        </button>
      ...
      {totalDays > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
          <div className="flex justify-between text-sm text-gray-500 font-medium">
            <span>₹{car.price.toLocaleString()} x {totalDays} days</span>
